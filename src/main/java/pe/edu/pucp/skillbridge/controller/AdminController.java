...
public class AdminController {
    ...
    public String inicio(Model model) {
        ...
        model.addAttribute("titulo", "Centro de administración");
    ...
    public String usuarios(@RequestParam(value = "q", required = false) String q, Model model) {
        ...
        cargarUsuarios(model, lista, q);
        return "admin/usuarios";
    }

    private void cargarUsuarios(Model model, List<Usuario> lista, String q) {
    ...
    public String guardarUsuario(Usuario usuario, @RequestParam("idRol") Integer idRol, Model model) {
        Rol rol = rolRepository.findById(idRol).orElseThrow();
        usuario.setRol(rol);
        Usuario guardado = usuarioRepository.save(usuario);

        // Modelo final: un Usuario puede tener como máximo un perfil Colaborador.
        ...
        registrarAuditoria("Guardar usuario", "Administración",
                "Se registró o actualizó la cuenta de " + guardado.getNombreCompleto());
        cargarUsuarios(model, usuarioRepository.findAll(), null);
        model.addAttribute("mensajeExito", "Usuario guardado correctamente.");
        return "admin/usuarios";
    }

    // Baja lógica: evita borrar físicamente registros relacionados.
    @PostMapping("/usuarios/{id}/estado")
    public String cambiarEstadoUsuario(@PathVariable Integer id, Model model) {
        Usuario usuario = usuarioRepository.findById(id).orElseThrow();
        usuario.setEstado(usuario.getEstado() == Usuario.EstadoUsuario.ACTIVO
                ? Usuario.EstadoUsuario.INACTIVO
                : Usuario.EstadoUsuario.ACTIVO);
        usuarioRepository.save(usuario);
        registrarAuditoria("Cambiar estado de usuario", "Administración",
                usuario.getNombreCompleto() + " quedó en estado " + usuario.getEstado());
        cargarUsuarios(model, usuarioRepository.findAll(), null);
        model.addAttribute("mensajeExito", "Estado del usuario actualizado.");
        return "admin/usuarios";
    }
    ...
    public String habilidades(@RequestParam(value = "q", required = false) String q, Model model) {
        ...
        cargarHabilidades(model, habilidades, q);
        return "admin/habilidades";
    }

    private void cargarHabilidades(Model model, List<Habilidad> habilidades, String q) {
        ...
        model.addAttribute("categorias", habilidades.stream().map(Habilidad::getCategoria)
    ...
    public String guardarHabilidad(Habilidad habilidad, Model model) {
        habilidadRepository.save(habilidad);
        registrarAuditoria("Guardar habilidad", "Habilidades", "Se actualizó el catálogo: " + habilidad.getNombre());
        cargarHabilidades(model, habilidadRepository.findAll(), null);
        model.addAttribute("mensajeExito", "Habilidad guardada correctamente.");
        return "admin/habilidades";
    }

    @PostMapping("/habilidades/{id}/estado")
    public String cambiarEstadoHabilidad(@PathVariable Integer id, Model model) {
        Habilidad habilidad = habilidadRepository.findById(id).orElseThrow();
        habilidad.setEstado(!Boolean.TRUE.equals(habilidad.getEstado()));
        habilidadRepository.save(habilidad);
        cargarHabilidades(model, habilidadRepository.findAll(), null);
        model.addAttribute("mensajeExito", "Estado de la habilidad actualizado.");
        return "admin/habilidades";
    }
    ...
    public String categorias(Model model) {
        ...
        model.addAttribute("titulo", "Categorías de habilidades");
    ...
    private void registrarAuditoria(String accion, String modulo, String detalle) {
        Usuario admin = usuarioRepository.findFirstByRolNombreOrderByIdUsuarioAsc("ADMINISTRADOR").orElse(null);
